...
start-frontends() {
  ...
  sep
  command "Starting Producer FRONTEND"
  sep

  kubectl create configmap producer-frontend-env-file --from-env-file=$K8S/producer-frontend/producer-frontend-env-file.properties -n supply-chain-network --save-config
  kubectl get configmap producer-frontend-env-file -o yaml > $K8S/producer-frontend/producer-frontend-configmap.yml -n supply-chain-network
